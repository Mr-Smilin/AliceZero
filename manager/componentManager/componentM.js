...
exports.GetMykiritoHelpMessage = (ephemeral = false) => {
	try {
		...
		returnMessage.setEphemeral(ephemeral).addEmbed(embed);
		return returnMessage.toMessage();
	} catch (err) {
...
exports.GetMusicAliceMessage = (ephemeral = false) => {
	try {
		...
		returnMessage.setEphemeral(ephemeral).addComponents(buttonComponents);
		return returnMessage.toMessage();
	} catch (err) {
...
exports.GetMyKiritoSkillMessage = (roleData, status = 0) => {
	try {
		...
		returnMessage.addEmbed(embed).addComponents(buttonComponents);
		return returnMessage.toMessage();
	} catch (err) {
...
exports.GetPrivacyStatementMessage = (ephemeral = true) => {
	try {
		const returnMessage = BDB.MNewMessage();
		let embed = embedC.PrivacyStatementMessage();
		returnMessage.setEphemeral(ephemeral).addEmbed(embed);
		return returnMessage.toMessage();
	} catch (err) {
		CatchF.ErrorDo(err, "GetPrivacyStatementMessage 方法異常!");
	}
};
